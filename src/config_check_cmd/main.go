...
func loadConfigs(
	allConfigs []config.RateLimitConfigToLoad, mergeDomainConfigs bool,
) {
	defer func() {
		err := recover()
		if err != nil {
			fmt.Printf(
				"error loading rate limit configs: %s\n", err.(error).Error(),
			)
			os.Exit(1)
		}
	}()
...
func main() {
	finalPath := "/home/stase/GolandProjects/diploma/ratelimit/examples" +
		"/ratelimit/config/example.yaml"
	...
	fmt.Printf(strconv.Itoa(len(configYaml.Descriptors)))
	fmt.Println(configYaml.StaticGroups[0].Metricsql)
	fmt.Println(configYaml.DynamicGroups[0].Descriptors[0].LimitMultiplier)
